...
function isSupportBranchSheet(sheetName) {
  const normalizedName = normalizeArabicSheetName(sheetName);
  return normalizedName.includes(normalizeArabicSheetName('فرع الدعم'))
    || normalizedName === normalizeArabicSheetName('الدعم والرعاية');
}

function getSupportSheetTitle(sheetName) {
  if (isSupportBranchSheet(sheetName)) {
    return 'فرع الدعم والرعاية';
  }

  return `الدعم والرعاية - مركز ${String(sheetName || '').trim() || 'الدعم'}`;
}
...
function buildWorkbook(sheets, filter) {
  ...
  sheets.forEach((sheet) => {
    ...
    const sheetData = [
      [getSupportSheetTitle(sheet.name), ...Array(rowWidth - 1).fill('')],
      headerRow,
      ...exportRows.map((row) => (
        filter.type === 'all'
...
function getUnifiedPrintSections() {
  const rows = processedSheets
    .filter((sheet) => isSupportBranchSheet(sheet.name))
...
window.getUnifiedSupportCentersPrintSections = getUnifiedSupportCentersPrintSections;
...
downloadButton.addEventListener('click', () => {
  ...
  XLSX.writeFile(outputWorkbook, `فرع ومراكز الدعم والرعاية (${selectedDateLabel}).xlsx`);
});
